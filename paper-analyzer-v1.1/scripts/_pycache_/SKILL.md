...
The skill uses a single combined interactive prompt to gather user preferences upfront. It uses the `question` tool's multiple-choice format with two questions in one call. **This prompt is presented after input reception but before any content extraction or analysis begins.**

### Combined Prompt: Language and Output Format Selection

**Timing**: After input reception, immediately before any content extraction or analysis.

**Purpose**: Set both the output language and the output delivery format in a single step, eliminating redundant interruptions.
...
- `生成 PDF 报告`: Set `[OUTPUT=pdf]`. The executor will run full analysis and generate a PDF report at the end. Report the full file path to the user upon completion.
- `控制台显示`: Set `[OUTPUT=console]`. The executor will run full analysis and output the complete Markdown report directly in the console at the end.

**Enforcement (MANDATORY)**:
1. After the user answers the combined prompt, the executor MUST record both choices as visible variables `[LANG=zh]` or `[LANG=en]` and `[OUTPUT=pdf]` or `[OUTPUT=console]` at the start of the analysis.
...
3. Both variables persist through the entire session. All output (console or PDF) MUST respect both `[LANG]` and `[OUTPUT]`.

> **Failure mode addressed**: Previously, two sequential prompts interrupted the workflow and were easily skipped. Merging them into a single upfront call reduces interruption count and ensures both decisions are made before any computation.
...
--- [MANDATORY: Record `[LANG=zh]` or `[LANG=en]` and `[OUTPUT=pdf]` or `[OUTPUT=console]` as visible variables] ---
...
3. Structured Analysis
 ...
 4. Output Generation
    ...
    ├── If `[OUTPUT=console]`:
    │   └── Output the Markdown directly in console (all dimensions), in `[LANG]`
    ├── If `[OUTPUT=pdf]`:
