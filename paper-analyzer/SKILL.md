---
name: paper-analyzer
description: Analyze academic papers from local PDF files, URLs (especially Arxiv), or direct text input, producing structured key points and detailed analysis reports covering research motivation, contributions, methodological innovations, experimental framework, and limitations. Supports optional chart/figure extraction from PDF and configurable analysis depth. Use this skill whenever the user asks to analyze, summarize, review, or deconstruct a paper; provides a local PDF path, an Arxiv URL, or any research article link; or needs structured paper insights for literature browsing, quality assessment, or building personal research directions.
---
...
- **Format per experiment**:
  ...
  ```
- **Must include**:
  - Dataset description (name, scale, domain)
  - Baselines compared (which specific methods, fairness of comparison)
  - Key metrics and numerical results
  - Any notable experiment design details (training schedule, hyperparameters)
...
The skill uses two interactive prompts to gather user preferences at key moments. Both use the `question` tool's multiple-choice format.

### Prompt 1: Language Selection

**Timing**: After input reception and content extraction, immediately before Structured Analysis.

**Purpose**: Set the output language for the analysis report. This affects all generated text (narrative, tables, assessments).

**Implementation**:
```tool
question:
  - header: "Analysis Language"
    question: "请选择分析报告使用的语言 / Please select output language / 言語を選択してください"
    options:
      - label: "中文 (Chinese)"
...
1. After user answers the language prompt, the executor MUST record the language choice as a visible variable `[LANG=zh]` or `[LANG=en]` at the start of the analysis.
2. EVERY section header, paragraph, bullet, and table cell containing narrative text MUST be written in `[LANG]`. If a section is found to violate this rule, the executor MUST rewrite it before proceeding.
3. The language variable `[LANG]` persists through the entire session — both console output AND PDF generation MUST respect it.
4. If the user later requests a file output (Markdown / PDF), that file MUST also be in the same `[LANG]`.

> **Failure mode addressed**: Previous executions selected `zh` but wrote analysis in English. This enforcement rule makes language tracking explicit and auditable at every output step.

### Prompt 2: Output Format Confirmation

**Timing**: After analysis is complete, before delivering output.

**Purpose**: Let the user choose between a PDF report file or console display.

**Implementation**:
```tool
question:
  - header: "Output Format"
    question: "选择输出方式 / Choose output format"
    options:
      - label: "生成 PDF 报告"
        description: "保存为 PDF 文件并告知路径，包含完整分析 + 实验流程图"
      - label: "控制台显示"
        description: "直接在控制台输出完整分析报告（Markdown格式）"
```

**Handling**:
- `PDF`: Generate PDF report in the current working directory, named `{PaperTitle}_Analysis_Report.pdf`. Report the full file path to the user upon completion.
- `Console`: Output the complete analysis report as Markdown directly in the console, organized by analysis dimensions.

**Important**: Do NOT skip these prompts. They must be presented in order at the specified points in the workflow.
...
--- [Interactive Prompt 1: Language Selection] ---
--- [MANDATORY: Record `[LANG=zh]` or `[LANG=en]` as visible variable] ---
   (This variable MUST be displayed at the top of the analysis and referenced before every section)
...
--- [Interactive Prompt 2: Output Format Confirmation] ---
...
4. Output Generation
   ...
   │   ├── All text in PDF MUST be in `[LANG]` (same as the analysis above)
   │   ├── Call the PDF script with `--lang=[LANG]` parameter (e.g., `--lang=zh`)
   │   ├── Include experimental framework flowchart
   │   └── Report full file path to user
   ├── If Console:
   │   └── Output full Markdown report in console (all dimensions), in `[LANG]`
...
1. **Do not copy paper section order**. Reorganize around the argument spine.
2. **Every experiment must answer a question**. If unclear, flag it.
3. **Evidence strength matters**. Distinguish what the paper proves vs. merely suggests.
4. **Respect user exclusions**. If user says skip a dimension, skip it completely.
5. **Be critical but fair**. Identify limitations without strawman arguments.
6. **Language compliance is MANDATORY, not a suggestion**. After the language prompt, `[LANG]` is explicitly set. ALL output — console narrative, file content, PDF text — MUST be in `[LANG]`. Technical terms (model names, dataset names, metrics, equations, code symbols) are always preserved in English regardless of `[LANG]`. If the user is Chinese-speaking and no prompt was shown, default to `[LANG=zh]`. **A section written in the wrong language is a bug and must be fixed before delivery.**
