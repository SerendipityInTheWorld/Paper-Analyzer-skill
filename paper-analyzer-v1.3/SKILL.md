...
3. Structured Analysis
   ...
   ├── Respect user's include/exclude and depth settings
    ...
    └── [GATE CHECK] MANDATORY — run `_gate_check(analysis_data)` before rendering:
        ├── Gate A: Every section must have content (paragraphs, bullets, or structured items)
        ├── Gate B: Language compliance verified (CJK ratio check)
        ├── Gate C: Each experiment must have `question`, `conclusion`, and `evidence`
        ├── If ANY gate fails → abort output, report exact errors to user, do NOT generate file
        └── Gate check is enforced by `build_report()` — rendering cannot proceed without passing
